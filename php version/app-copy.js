...
// Mapas Base
const mapaCalle = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", { maxZoom: 19, crossOrigin: true });
const satelite = L.layerGroup([
  ...
  L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}', { maxZoom: 19, crossOrigin: true })
]);

const map = L.map("map", { preferCanvas: false, doubleClickZoom: false, zoomControl: false, layers: [mapaCalle] }).setView(TUBRICA_LOCATION, 13);
L.control.layers({ "Mapa": mapaCalle, "Satélite": satelite }, null, { position: 'topright' }).addTo(map);
...
L.marker(TUBRICA_LOCATION, { interactive: false }).addTo(map).bindPopup("<b>📍 SEDE TUBRICA</b>").openPopup();

async function init() {
  cargarMunicipios();
  await loadData(); // Carga de PHP
  await cargarRutasOmega();
}
init();

// ==========================================
// 2. RENDERIZADO Y PERSISTENCIA (PHP)
// ==========================================
...
function renderRoute(latlngs, name, id, isOmega = false) {
  if (!latlngs || latlngs.length < 2) return;
  const poly = L.polyline(latlngs, { color: isOmega ? "#e67e22" : "#3498db", weight: 6, smoothFactor: 1.5 });
  ...
  const update = (pts) => {
    ...
    label.setIcon(L.divIcon({
      ...
      html: `<div class="route-text"><span class="route-name">${name}</span><span class="route-stats">${(m/1000).toFixed(1)}km | ${Math.round(m/11.11/60)}m</span></div>`,
      iconAnchor: [0, 0]
    }));
...
// ==========================================
// 3. HERRAMIENTAS (OMEGA, SMART, PDF)
// ==========================================
async function cargarRutasOmega() {
  ...
  for (let i = 0; i < OMEGA_WAYPOINTS.length; i++) {
    ...
    const coordsStr = route.points.map(p => `${p[1]},${p[0]}`).join(';');
    ...
    if (data.routes) {
      ...
      const layer = renderRoute(poly, route.name, `omega-${i}`, true);
      const item = document.createElement("div");
      item.className = "omega-item";
      item.innerHTML = `<input type="checkbox" checked onchange="toggleOmegaLayer(${i}, this.checked)"><span onclick="focusRoute(${i})">${route.name}</span>`;
...
window.toggleOmegaLayer = (i, show) => show ? omegaLayer.addLayer(OMEGA_WAYPOINTS[i].layer) : omegaLayer.removeLayer(OMEGA_WAYPOINTS[i].layer);
window.toggleAllOmega = (show) => {
  document.querySelectorAll(".omega-item input").forEach((chk, i) => { chk.checked = show; toggleOmegaLayer(i, show); });
};
...
// [Funciones Auxiliares: Municipios, Borrador, PDF...]
async function cargarMunicipios() {
  ...
  zonas.forEach(z => {
    fetch(`https://nominatim.openstreetmap.org/details.php?osmtype=R&osmid=${z.id}&class=boundary&format=json&polygon_geojson=1`)
      ...
      if (data.geometry) L.geoJSON(data.geometry, { interactive: false, style: { color: z.col, weight: 3, opacity: 0.6, fillColor: z.col, fillOpacity: 0.08 } }).addTo(boundaryLayer);
    });
  });
}
...
map.on('mousemove', e => {
  ...
  [drawnItems, omegaLayer].forEach(lg => lg.eachLayer(g => {
    if (g.type !== 'route') return;
    const pts = g.routeLine.getLatLngs();
    const filt = pts.filter(p => map.distance(p, e.latlng) > eraserSize);
    if (filt.length !== pts.length) { g.routeLine.setLatLngs(filt); g.isDirty = true; }
  }));
});

map.on('mouseup', () => { if (currentMode === 'eraser') drawnItems.eachLayer(g => { if (g.isDirty) saveLayer(g); }); });

window.exportMapToPDF = () => {
  const btn = document.querySelector(".btn-pdf"); btn.innerHTML = "⏳ Generando...";
  setTimeout(() => {
    html2canvas(document.getElementById("map"), { useCORS: true, scale: 3 }).then(canvas => {
      const pdf = new jspdf.jsPDF('l', 'mm', 'a4');
      pdf.addImage(canvas.toDataURL("image/png"), 'PNG', 0, 0, 297, 210);
      pdf.save("Rutas_Tubrica.pdf");
...
window.enableManualDraw = () => { resetModes(); currentMode = 'manual'; new L.Draw.Polyline(map, { shapeOptions: { color: '#3498db', weight: 6 } }).enable(); };
...
window.toggleGoToTubrica = () => { resetModes(); currentMode = 'to-tubrica'; };
window.enableMarker = () => { resetModes(); currentMode = 'marker'; };
window.enableEditMode = () => { resetModes(); currentMode = 'edit'; };
window.enableDelete = () => { resetModes(); currentMode = 'delete'; };
window.toggleEraser = () => { resetModes(); currentMode = 'eraser'; eraserCircle = L.circle([0,0], { radius: eraserSize, color: "red", fillOpacity: 0.2 }).addTo(map); };
function renderMarker(l, n, id) { const m = L.marker(l).bindPopup(n); m.dbId = id; m.on('click', e => { L.DomEvent.stopPropagation(e); if (currentMode === "delete") handleDelete(m); }); m.addTo(drawnItems); }
async function handleDelete(l) { if (confirm("¿Borrar?")) { if(l.dbId) await fetch(`api.php?id=${l.dbId}`, { method: 'DELETE' }); drawnItems.removeLayer(l); omegaLayer.removeLayer(l); } }
function resetModes() { currentMode = null; smartRoutePoints = []; [drawnItems, omegaLayer].forEach(lg => lg.eachLayer(g => { if(g.routeLine?.editing) g.routeLine.editing.disable(); })); tempMarkers.forEach(m => map.removeLayer(m)); tempMarkers = []; if (previewLine) map.removeLayer(previewLine); if (eraserCircle) { map.removeLayer(eraserCircle); eraserCircle = null; } document.getElementById("status").innerText = "Modo: Inactivo"; }
async function fetchOSRMDirect(p) { const res = await fetch(`https://router.project-osrm.org/route/v1/driving/${p.map(x=>`${x.lng},${x.lat}`).join(';')}?overview=full&geometries=geojson`); const data = await res.json(); if (data.routes) { const pts = data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]); const n = prompt("Nombre:"); if (n) renderRoute(pts, n, await saveElement(n, 'route', pts)); } resetModes(); }
async function finalizeSmart() { const res = await fetch(`https://router.project-osrm.org/route/v1/driving/${smartRoutePoints.map(p=>`${p.lng},${p.lat}`).join(';')}?overview=full&geometries=geojson`); const data = await res.json(); if (data.routes) { const pts = data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]); const n = prompt("Nombre:"); if (n) renderRoute(pts, n, await saveElement(n, 'route', pts)); } resetModes(); }
async function updatePreview() { fetch(`https://router.project-osrm.org/route/v1/driving/${smartRoutePoints.map(p=>`${p.lng},${p.lat}`).join(';')}?overview=full&geometries=geojson`).then(r => r.json()).then(data => { if (previewLine) map.removeLayer(previewLine); if (data.routes) previewLine = L.polyline(data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]), { color: '#e67e22', weight: 4, dashArray: '5, 10' }).addTo(map); }); }
...
window.saveAllChanges = async () => { const p = []; drawnItems.eachLayer(l => { if (l.isDirty) p.push(saveLayer(l)); }); await Promise.all(p); alert("Sincronizado"); };