...
function updateLegend() {
  ...
  const allLayers = [];
  omegaLayer.eachLayer((l) => {
    if (l.routeLine) allLayers.push(l);
  });
  ...
  if (allLayers.length === 0) {
    container.innerHTML =
      '<p style="color:#aaa;font-size:12px;padding:8px;">Sin rutas activas</p>';
    return;
  }
  ...
  allLayers.forEach((group) => {
    ...
    const dist = group.totalDistance
      ...
      : "—";
    const time = group.totalTime
      ? Math.round(group.totalTime / 60) + " min"
      : "—";
